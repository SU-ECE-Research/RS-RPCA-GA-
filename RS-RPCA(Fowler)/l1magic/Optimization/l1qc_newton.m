...
while (~done)
  ...
  coneiter = 0;
  while ( (max(abs(xp)-up) > 0) | (rp'*rp > epsilon^2) )
    s = beta*s;
    xp = x + s*dx;  up = u + s*du;  rp = r + s*Adx;
    coneiter = coneiter + 1;
    if (coneiter > 32)
...
%   disp(sprintf('Newton iter = %d, Functional = %8.3f, Newton decrement = %8.3f, Stepsize = %8.3e, Cone iterations = %8.3f, Backtrack iterations = %d', ...
